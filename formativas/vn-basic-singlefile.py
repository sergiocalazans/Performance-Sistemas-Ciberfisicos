# von Neumann - Arquitetura Básica
# Todos as classes em um mesmo arquivo
# PSCF - Prof. Luiz Lima Jr.
#
# Arquitetura formada de 3 componentes básicos:
#
# 1. Memória => RAM
# 2. CPU
# 3. Entrada e Saída (IO)


class IO:
    def output(self, s):
        print(s, end='')

    def input(self, prompt):
        return input(prompt)


# Exceção (erro)
class EnderecoInvalido(Exception):
    def __init__(self, ender):
        self.ender = ender

    def __repr__(self):     # para fazer print
        return self.ender


class RAM:
    def __init__(self, k):
        self._tam = 2**k
        self.memoria = [0] * self._tam

    def capacidade(self):
        return self._tam

    def verifica_endereco(self, ender):
        if (ender < 0) or (ender >= self._tam):
            raise EnderecoInvalido(ender)   # lança exceção (erro)

    def read(self, ender):
        self.verifica_endereco(ender)
        return self.memoria[ender]

    def write(self, ender, val):
        self.verifica_endereco(ender)
        self.memoria[ender] = val


class CPU:
    def __init__(self, mem, io):
        self.mem = mem
        self.io = io
        self.PC = 0  # program counter
        self.A = self.B = self.C = 0

    def run(self, ender):
        self.PC = ender
        self.A = self.mem.read(self.PC)
        self.PC += 1
        self.B = self.mem.read(self.PC)
        self.PC += 1

        self.C = 1
        while self.A <= self.B:
            self.mem.write(self.A, self.C)
            self.io.output(f'> {self.A} = {self.C}\n')
            self.C += 1
            self.A += 1

class CACHE:

    def __init__(self, m, k):
        self.m = 2**m
        self.k = 2**k

    def iniciar(self):
        self.cache = [[[0] * self.k], 0, False] * self.m
        return print(self.cache[0][0])


def main():
    try:
        #io = IO()
        #ram = RAM(7)
        #cpu = CPU(ram, io)
        cache = CACHE(7, 5)
        cache.iniciar()

        #ram.write(10, 120)
        # ram.write(11, 130)
        # cpu.run(10)

    except EnderecoInvalido as e:
        print(f'Endereço inválido: {e}')


if __name__ == '__main__':
    main()
